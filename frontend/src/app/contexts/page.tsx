      ...
      {/* Top Header */}
        ...
        <div className="mt-8 flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-800">
          <div className="flex flex-wrap items-center bg-slate-950 p-1 rounded-xl border border-slate-800 text-xs">
            ...
            <button
              type="button"
              onClick={() => setStageFilter("initial")}
            ...
            >
              ...
              <span>Paso 1: Directivas</span>
            </button>
            <button
              type="button"
              onClick={() => setStageFilter(isValidator ? "validate_text" : "waiting_ctx_val")}
              className={`px-3 py-1.5 rounded-lg font-medium transition-all flex items-center gap-1.5 ${
                stageFilter === "waiting_ctx_val" || stageFilter === "validate_text"
                  ? "bg-amber-600 text-white shadow-sm"
                  : "text-slate-400 hover:text-white"
              }`}
            >
              <Clock className="w-3.5 h-3.5" />
              <span>Paso 2: {isValidator ? "Validar Directivas" : "En Espera Validación"} ({contexts.filter((c) => c.status === "PENDING").length})</span>
            </button>
            <button
              type="button"
              onClick={() => setStageFilter(isValidator ? "validate_plan" : "waiting_ctx_plan")}
              className={`px-3 py-1.5 rounded-lg font-medium transition-all flex items-center gap-1.5 ${
                stageFilter === "waiting_ctx_plan" || stageFilter === "validate_plan" || stageFilter === "plan"
                  ? "bg-purple-600 text-white shadow-sm"
                  : "text-slate-400 hover:text-white"
              }`}
            >
              <Bot className="w-3.5 h-3.5" />
              <span>Paso 3: {isValidator ? "Validar Plan" : "Plan en Espera"} ({contexts.filter((c) => c.status === "PLAN_PENDING" || c.status === "APPROVED").length})</span>
            </button>
            <button
              type="button"
            ...
            >
              <CheckCircle2 className="w-3.5 h-3.5" />
              <span>Paso 4: Aceptados / Activos ({contexts.filter((c) => c.status === "ACCEPTED").length})</span>
            </button>
          </div>
        </div>

        {/* Step-Specific Guidance Banner when filter is active */}
        {stageFilter === "initial" && (
          <div className="mt-4 p-4 rounded-2xl bg-gradient-to-r from-indigo-900/30 via-slate-900 to-indigo-950/20 border border-indigo-500/30 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-start gap-3">
              ...
              <div className="text-xs sm:text-sm text-slate-200">
                <div className="flex items-center gap-2">
                  ...
                  <strong className="text-indigo-300 font-semibold">Creación de Directivas de Contexto</strong>
                </div>
                <p className="mt-1 text-slate-300 text-xs leading-relaxed">
                  Redacta las normas de arquitectura, estilo y librerías clave. Al enviar, el validador del repositorio asignado revisará y aprobará el texto antes de generar el plan técnico con Gemini.
                </p>
              </div>
            </div>
            <button
              onClick={openCreateModal}
              className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold shadow-lg shadow-indigo-600/30 shrink-0 transition-all"
            >
              <Plus className="w-4 h-4" />
              {t("contexts.new_context_button", "Nuevo Contexto")}
            </button>
        ...
        {(stageFilter === "waiting_ctx_val" || (stageFilter === "validate_text" && !isValidator)) && (
          <div className="mt-4 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 shadow-sm flex items-start gap-3">
            ...
            <div className="flex-1 text-xs sm:text-sm text-slate-200">
              <div className="flex items-center gap-2">
                ...
                <strong className="text-amber-300 font-semibold">En Espera de Validación de Directivas</strong>
              </div>
              <p className="mt-1 text-slate-300 text-xs leading-relaxed">
                Tus directivas han sido enviadas. Al no ser validador de este repositorio, debes esperar a que el validador o administrador revise y apruebe el texto inicial antes de que Gemini pueda generar el Plan Técnico. No requieres realizar ninguna acción en este paso.
              </p>
            </div>
          </div>
        )}

        {stageFilter === "validate_text" && isValidator && (
          <div className="mt-4 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-start gap-3">
              ...
              <div className="text-xs sm:text-sm text-slate-200">
                <div className="flex items-center gap-2">
                  ...
                  <strong className="text-amber-300 font-semibold">Validación de Directivas de Contexto</strong>
                </div>
                <p className="mt-1 text-slate-300 text-xs leading-relaxed">
                  Como validador, eres responsable de revisar y autorizar las directivas técnicas de tu equipo para asegurar su calidad antes de activar la generación del Plan Técnico con Gemini.
                </p>
              </div>
            </div>
            <Link
              href="/validator?section=contexts"
              className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-white text-xs font-semibold shadow-md shrink-0 transition-all"
            >
              <CheckSquare className="w-4 h-4" />
              Ir a Panel Validador
            </Link>
          </div>
        )}

        {(stageFilter === "waiting_ctx_plan" || (stageFilter === "validate_plan" && !isValidator) || stageFilter === "plan") && (
          <div className="mt-4 p-4 rounded-2xl bg-purple-500/10 border border-purple-500/30 shadow-sm flex items-start gap-3">
            ...
            <div className="flex-1 text-xs sm:text-sm text-slate-200">
              <div className="flex items-center gap-2">
                ...
                <strong className="text-purple-300 font-semibold">Plan de Contexto (En Espera de Autorización)</strong>
              </div>
              <p className="mt-1 text-slate-300 text-xs leading-relaxed">
                Gemini ha estructurado el Plan Técnico para este contexto (o se encuentra procesándolo en segundo plano). El validador debe dar su autorización final para que el contexto quede aceptado y disponible con caché.
              </p>
            </div>
          </div>
        )}

        {stageFilter === "validate_plan" && isValidator && (
          <div className="mt-4 p-4 rounded-2xl bg-purple-500/10 border border-purple-500/30 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-start gap-3">
              <Bot className="w-5 h-5 text-purple-400 shrink-0 mt-0.5" />
              <div className="text-xs sm:text-sm text-slate-200">
                <div className="flex items-center gap-2">
                  ...
                  <strong className="text-purple-300 font-semibold">Validación de Plan Técnico de Contexto</strong>
                </div>
                <p className="mt-1 text-slate-300 text-xs leading-relaxed">
                  Revisa la estructuración técnica generada por Gemini y aprueba o solicita ajustes para activar el contexto en el sistema.
                </p>
              </div>
            </div>
            <Link
              href="/validator?section=contexts"
              className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-xs font-semibold shadow-md shrink-0 transition-all"
            >
              <CheckSquare className="w-4 h-4" />
        ...
        {stageFilter === "accepted" && (
          <div className="mt-4 p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 shadow-sm flex items-start gap-3">
            ...
            <div className="flex-1 text-xs sm:text-sm text-slate-200">
              <div className="flex items-center gap-2">
                ...
                <strong className="text-emerald-300 font-semibold">Contextos Aceptados y Activos</strong>
              </div>
              <p className="mt-1 text-slate-300 text-xs leading-relaxed">
                Estos contextos han superado ambas fases de validación y cuentan con caché activo en Gemini. Puedes utilizarlos inmediatamente en tus nuevos prompts para acelerar el desarrollo y reducir el consumo de tokens.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="mt-4 space-y-4">
          {loading && contexts.length === 0 ? (
            ...
            const filteredContexts = contexts.filter((ctx) => {
              if (stageFilter === "waiting_ctx_val" || stageFilter === "validate_text") return ctx.status === "PENDING";
              if (stageFilter === "waiting_ctx_plan" || stageFilter === "validate_plan" || stageFilter === "plan") {
              ...
              return (
                <div className="py-16 text-center rounded-2xl border border-dashed border-slate-800 bg-slate-900/30">
                  ...
                  <h3 className="text-base font-medium text-slate-300">
                    {stageFilter === "accepted" ? "No hay contextos aceptados todavía" : "No hay contextos en esta etapa"}
                  </h3>
                  <p className="text-xs text-slate-500 mt-1">
                    {stageFilter === "accepted"
                      ? "Crea un contexto y completa las dos fases de validación para activarlo con Gemini Cache."
                      : 'Selecciona "Todas las etapas" para visualizar todos tus contextos.'}
                  </p>
                </div>
              );
            }
            ...
            return (
              <div className="grid grid-cols-1 gap-4">
                ...
                <div
                  key={ctx.id}
                  className={`p-5 rounded-2xl transition-all shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 border ${
                    ctx.status === "ACCEPTED"
                      ? "bg-slate-900/95 border-emerald-500/40 hover:border-emerald-500/60 shadow-emerald-950/20"
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center gap-2 shrink-0 border-t md:border-t-0 pt-3 md:pt-0 border-slate-800">
                    {ctx.status === "ACCEPTED" && (
                      <Link
                        href={`/dashboard?context=@${ctx.identifier}`}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-xs font-semibold text-white transition-all shadow-sm shadow-emerald-600/30"
                        title="Crear un nuevo prompt utilizando este contexto aceptado"
                      >
                        <Sparkles className="w-3.5 h-3.5" />
                        <span>Usar en Nuevo Prompt</span>
                      </Link>
                    )}
