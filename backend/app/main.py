...
async def lifespan(app: FastAPI):
    ...
    async with engine.begin() as conn:
        ...
        try:
            ...
            await conn.execute(text("ALTER TABLE prompt_tasks ADD COLUMN IF NOT EXISTS tokens_fixed_context INTEGER DEFAULT 0;"))
            await conn.execute(text("ALTER TABLE prompt_tasks ADD COLUMN IF NOT EXISTS tokens_temporal_context INTEGER DEFAULT 0;"))
            await conn.execute(text("ALTER TABLE prompt_tasks ADD COLUMN IF NOT EXISTS temporal_context TEXT;"))
            await conn.execute(text("ALTER TABLE prompt_tasks ADD COLUMN IF NOT EXISTS temporal_context_status VARCHAR(50) DEFAULT 'ACTIVE';"))
            await conn.execute(text("ALTER TABLE user_token_logs ADD COLUMN IF NOT EXISTS tokens_fixed_context INTEGER DEFAULT 0;"))
            await conn.execute(text("ALTER TABLE user_token_logs ADD COLUMN IF NOT EXISTS tokens_temporal_context INTEGER DEFAULT 0;"))
