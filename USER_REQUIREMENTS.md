...
### FR-10: Administration Console (User, Repository & Model Governance)
- **Description:** Administrators must have total operational control over users, repository catalogs, and AI configurations.
- **Features:**
  ...
  - **GitHub Repositories Manager:** Register target project repositories, default branch names, and Personal Access Tokens (PAT). Active repositories immediately appear in the user prompt selector.
  - **Google Gemini AI Settings:** Inspect and hot-swap the active Gemini API Key and switch between available models (`gemini-2.5-flash`, `gemini-1.5-flash`, `gemini-1.5-pro`).
...
### FR-13: Dedicated Personal Pull Requests Center (`/pull-requests`)
- **Description:** A dedicated, personal workspace accessible to all authenticated users providing full traceability over their prompt-generated GitHub Pull Requests.
- **Features:**
  - Real-time catalog of all PRs initiated by the current user.
  - Search bar filtering by prompt text or task ID.
  - Project filter dropdown.
  - Copyable feature branch badge (`vibe/task-{id}-{hash}`) with one-click clipboard copy.
  - Direct external link button to open the Pull Request on GitHub.
  - Modal code diff inspector displaying the synthesized changes and files touched.
  - Modal execution log viewer displaying runner container output for full transparency.
...
| **NFR-05** | **Responsive Design** | The web interface (Next.js 15 + Tailwind CSS) must be fully responsive across desktop, tablet, and mobile browsers on port `3010` (or `80` via gateway). |
...
| **NFR-07** | **Data Persistence & Durability** | System data (users, prompts, invitations, projects, chat) must be persisted in a dedicated relational database (PostgreSQL 16) backed by a named Docker volume (`vibe_postgres_data`), guaranteeing complete immunity to container restarts or rebuilds. |
...
### Scenario 5: User Accesses Personal Pull Requests Dashboard on Port 3010
```gherkin
Given an authenticated user who has authored prompts with opened Pull Requests
When the user accesses "http://localhost:3010/pull-requests" (or via the navigation bar)
Then the system displays the "Mis Pull Requests" view
And the user can view the copyable feature branch badge, prompt description, and GitHub PR link
And clicking "Ver Diff" displays the code changes generated by Google Gemini
And clicking "Ver Logs" displays the Docker container execution logs.
```
